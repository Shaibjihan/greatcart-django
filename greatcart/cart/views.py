        ...
        if product_id  and  quantity :
            ...
            if int(quantity) > 0:
                if Product.in_stock:        
                    ...
                    messages.warning(request, "The prodcut is not in stock anymore.")
                    return redirect('cart')
            else:
                cart.update(int(product_id), int(quantity))
                return redirect('cart')
    ...
    def post(self, *args, **kwargs):
        ...
        if coupon.exists():
            ...
            if current_date < active_date:
                ...
                return redirect('cart')
            if cart.total()  < coupon.required_amout_to_use_coupon:
                messages.warning(self.request, f'You have to shop at least $ {coupon.required_amout_to_use_coupon} to use this coupon code')
                return redirect('cart')
